...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($idTimeCasa == $idTimeFora) {
        ...
        $stmt->bind_param("iisiii", $idTimeCasa, $idTimeFora, $dataPartida, $golsCasa, $golsFora, $id);
        if ($stmt->execute()) {
    ...
    <form action="" method="post">
        ...
        <select name="timeCasa">
            <?php while ($row = $resultTimes->fetch_assoc()): ?>
                <option value="<?= $row['id'] ?>" <?php if($dado['time_casa_id'] == $row['id']){echo 'selected';} ?>><?= $row['nome'] ?></option>
            <?php endwhile; ?>
        </select><br>
        <label for="timeFora">Time de Fora:</label>
        <select name="timeFora">
            <?php while ($row = $resultTimes2->fetch_assoc()): ?>
                <option value="<?= $row['id'] ?>" <?php if($dado['time_fora_id'] == $row['id']){echo 'selected';} ?>><?= $row['nome'] ?></option>
            <?php endwhile; ?>
        </select><br>
        <label for="data">Data da Partida:</label>
        <input type="date" name="data" id="data" value="<?=$dado['data_jogo']?>"> <br>
        <label for="golsCasa">Gols do time da Casa:</label>
        <input type="number" name="golsCasa" id="golsCasa"  value="<?=$dado['gols_casa']?>"><br>
        <label for="golsFora">Gols do time de Fora:</label>
        <input type="number" name="golsFora" id="golsFora"  value="<?=$dado['gols_fora']?>"><br>
